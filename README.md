...
4. Les binaires système nécessaires sont listés dans `packages.txt` (apt) : `ghostscript` et `poppler-utils` seront installés automatiquement sur Streamlit Cloud.
...
5. Consulte les logs Streamlit Cloud si un binaire manque malgré `packages.txt`.
6. Test local rapide :
   ```bash
   python3 -m venv .venv
